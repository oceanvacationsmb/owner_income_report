...
const SAVED_REPORTS_STORAGE_KEY = "owner_saved_reports_v1";
const TASKS_API_CANDIDATES = [
  (typeof window !== "undefined" && window.TASKS_API_URL) ? String(window.TASKS_API_URL).trim() : ""
].filter(Boolean);
const SAVED_REPORTS_API_CANDIDATES = [
  (typeof window !== "undefined" && window.SAVED_REPORTS_API_URL) ? String(window.SAVED_REPORTS_API_URL).trim() : "/api/saved-reports"
].filter(Boolean);
...
let savedReportsPersistenceMode = "local";
let savedReportsInitPromise = null;
let resolvedSavedReportsApiBase = "";
let activeSavedReportId = "";
let hasAttemptedSavedReportQueryLoad = false;
...
function normalizeRecurringCharges(items) {
  ...
  return items
    .map(item => {
      ...
      return {
        id: String(item.id || generateLocalId("charge")),
        name,
        amount
      };
    })
...
function normalizeOwnerStayFees(items) {
  if (!Array.isArray(items)) return [];
  return items
    .map(item => {
      if (!item || typeof item !== "object") return null;
      ...
      return {
        id: String(item.id || generateLocalId("owner_stay")),
        label,
        cleaningFee
      };
    })
...
function normalizeSavedReport(report) {
  ...
  const id = String(report.id || "").trim() || generateLocalId("report");
  ...
  const recurringCharges = normalizeRecurringCharges(report.recurringCharges || []);
  const ownerStayFees = normalizeOwnerStayFees(report.ownerStayFees || []);
...
function saveSavedReportsToStorage() {
  try {
    localStorage.setItem(SAVED_REPORTS_STORAGE_KEY, JSON.stringify(savedReportsData));
  } catch (_) {
    // best-effort persistence
  }
}

async function fetchSavedReportsFromApi() {
  const candidates = resolvedSavedReportsApiBase ? [resolvedSavedReportsApiBase] : SAVED_REPORTS_API_CANDIDATES;
  for (const base of candidates) {
    try {
      const res = await fetch(base, { headers: { accept: "application/json" } });
      if (!res.ok) continue;
      const payload = await res.json();
      const list = Array.isArray(payload?.reports) ? payload.reports : payload;
      if (!Array.isArray(list)) continue;
      resolvedSavedReportsApiBase = base;
      return list.map(normalizeSavedReport).filter(Boolean);
    } catch (_) {
      // continue
    }
  }
  throw new Error("saved-reports-api-unavailable");
}

async function ensureSavedReportsInitialized() {
  if (!(currentOwner && currentOwner.admin)) return savedReportsData;
  if (savedReportsInitPromise) return savedReportsInitPromise;

  savedReportsInitPromise = (async () => {
    try {
      const apiReports = await fetchSavedReportsFromApi();
      savedReportsData = apiReports;
      savedReportsPersistenceMode = "server";
      saveSavedReportsToStorage();
    } catch (_) {
      savedReportsPersistenceMode = "local";
      savedReportsData = loadSavedReportsFromStorage();
    }

    renderSavedReportsList();
    attemptLoadSavedReportFromQuery();
...
function getStatementEditorValues() {
  const statementDate = String(document.getElementById("statementDateInput")?.value || "").trim();
  const ownerName = String(document.getElementById("statementOwnerNameInput")?.value || "").trim();
  const ownerEmail = String(document.getElementById("statementOwnerEmailInput")?.value || "").trim().toLowerCase();
  const propertyName = String(document.getElementById("statementPropertyNameInput")?.value || "").trim();
  const periodLabel = String(document.getElementById("statementPeriodInput")?.value || "").trim();
  const ownerType = String(document.getElementById("statementOwnerTypeSelect")?.value || "payout").toLowerCase() === "draft" ? "draft" : "payout";

  const recurringCharges = Array.from(document.querySelectorAll("#recurringExpensesBody tr"))
    .map(row => {
      const name = String(row.querySelector(".recurring-name-input")?.value || "").trim();
      const amount = Number(row.querySelector(".recurring-amount-input")?.value) || 0;
      const id = String(row.getAttribute("data-row-id") || generateLocalId("charge"));
      if (!name && !amount) return null;
      return { id, name, amount };
    })
    .filter(Boolean);

  const ownerStayFees = Array.from(document.querySelectorAll("#ownerStayFeesBody tr"))
    .map(row => {
      const label = String(row.querySelector(".owner-stay-label-input")?.value || "").trim();
      const cleaningFee = Number(row.querySelector(".owner-stay-fee-input")?.value) || 0;
      const id = String(row.getAttribute("data-row-id") || generateLocalId("owner_stay"));
      if (!label && !cleaningFee) return null;
  ...
  return normalizeSavedReport({
    id: activeSavedReportId || generateLocalId("report"),
    statementDate,
    ownerName,
    ownerEmail,
    propertyName,
...
function createRecurringExpenseRow(item = {}) {
  const tbody = document.getElementById("recurringExpensesBody");
  if (!tbody) return;

  const tr = document.createElement("tr");
  tr.setAttribute("data-row-id", String(item.id || generateLocalId("charge")));
  tr.innerHTML = `
    <td>
      <input type="text" class="statement-input recurring-name-input" placeholder="Expense name" value="${escapeHtml(String(item.name || ""))}" />
    </td>
    <td>
      <input type="number" step="0.01" min="0" class="statement-input recurring-amount-input" value="${Number(item.amount) || 0}" />
    </td>
    <td class="statement-action-cell">
      <button type="button" class="statement-mini-btn statement-mini-btn-danger recurring-delete-btn">Delete</button>
    </td>
  `;
  ...
  if (deleteBtn) {
    deleteBtn.onclick = () => {
      tr.remove();
      updateRecurringTotalDisplay();
    };
  }

  const amountInput = tr.querySelector(".recurring-amount-input");
  if (amountInput) {
    amountInput.oninput = () => updateRecurringTotalDisplay();
  }
}

function createOwnerStayFeeRow(item = {}) {
  const tbody = document.getElementById("ownerStayFeesBody");
  if (!tbody) return;

  const tr = document.createElement("tr");
  tr.setAttribute("data-row-id", String(item.id || generateLocalId("owner_stay")));
  tr.innerHTML = `
    <td>
      <input type="text" class="statement-input owner-stay-label-input" placeholder="Owner stay" value="${escapeHtml(String(item.label || ""))}" />
    </td>
    <td>
      <input type="number" step="0.01" min="0" class="statement-input owner-stay-fee-input" value="${Number(item.cleaningFee) || 0}" />
    </td>
    <td class="statement-action-cell">
      <button type="button" class="statement-mini-btn statement-mini-btn-danger owner-stay-delete-btn">Delete</button>
    </td>
  `;

  tbody.appendChild(tr);

  const deleteBtn = tr.querySelector(".owner-stay-delete-btn");
  if (deleteBtn) {
    deleteBtn.onclick = () => {
      tr.remove();
      updateOwnerStayTotalDisplay();
    };
  }

  const feeInput = tr.querySelector(".owner-stay-fee-input");
  if (feeInput) {
    feeInput.oninput = () => updateOwnerStayTotalDisplay();
  }
}

function updateOwnerStayTotalDisplay() {
  const total = Array.from(document.querySelectorAll("#ownerStayFeesBody .owner-stay-fee-input"))
    .reduce((sum, input) => sum + (Number(input.value) || 0), 0);
  const totalEl = document.getElementById("ownerStayFeesTotal");
  if (totalEl) totalEl.textContent = formatMoney(total);
}

function getScannedOwnerStayFeesFromReservations() {
  return getFilteredOwnerStays().map(stay => {
    const checkIn = formatDateDisplay(stay.checkIn || stay.checkInDate || "");
    const checkOut = formatDateDisplay(stay.checkOut || stay.checkOutDate || "");
    const nights = Number(stay.numberOfNights) || 0;
    const nightsLabel = nights > 0 ? ` (${nights}N)` : "";
    const label = `${checkIn} - ${checkOut}${nightsLabel}`.trim();
    return {
      id: generateLocalId("owner_stay"),
      label,
      cleaningFee: Number(getCleaningFee()) || 0
    };
  });
}

function refillOwnerStayFeesFromScan() {
  const tbody = document.getElementById("ownerStayFeesBody");
  if (!tbody) return;
  tbody.innerHTML = "";
  const scanned = getScannedOwnerStayFeesFromReservations();
  if (!scanned.length) {
    createOwnerStayFeeRow({ label: "", cleaningFee: Number(getCleaningFee()) || 0 });
  } else {
    scanned.forEach(item => createOwnerStayFeeRow(item));
  }
  updateOwnerStayTotalDisplay();
}

function toggleOwnerStaySection(ownerType, isExistingReport) {
  const section = document.getElementById("ownerStayFeesSection");
  const scanBtn = document.getElementById("statementScanOwnerStaysBtn");
  if (!section) return;
  const isPayout = String(ownerType || "payout") === "payout";
  section.style.display = isPayout ? "" : "none";
  if (scanBtn) scanBtn.disabled = !isPayout;

  if (!isPayout) return;

  const existingRows = Array.from(document.querySelectorAll("#ownerStayFeesBody tr"));
  if (!isExistingReport && existingRows.length === 0) {
    refillOwnerStayFeesFromScan();
  }
}

function updateRecurringTotalDisplay() {
  const total = Array.from(document.querySelectorAll("#recurringExpensesBody .recurring-amount-input"))
    .reduce((sum, input) => sum + (Number(input.value) || 0), 0);
  const totalEl = document.getElementById("recurringExpensesTotal");
  if (totalEl) totalEl.textContent = formatMoney(total);
}

function populateStatementEditor(report = null) {
  const ownerNameInput = document.getElementById("statementOwnerNameInput");
  const ownerEmailInput = document.getElementById("statementOwnerEmailInput");
  const propertyInput = document.getElementById("statementPropertyNameInput");
  const dateInput = document.getElementById("statementDateInput");
  const periodInput = document.getElementById("statementPeriodInput");
  const ownerTypeSelect = document.getElementById("statementOwnerTypeSelect");
  const tbody = document.getElementById("recurringExpensesBody");
  const ownerStayTbody = document.getElementById("ownerStayFeesBody");
  const editorTitle = document.getElementById("statementEditorTitle");

  if (!ownerNameInput || !ownerEmailInput || !propertyInput || !dateInput || !periodInput || !ownerTypeSelect || !tbody || !ownerStayTbody || !editorTitle) {
    return;
  }
  ...
  const selected = report || {
    ...
    propertyName: currentOwner?.propertyName || "",
  ...
  activeSavedReportId = report?.id || "";
  editorTitle.textContent = activeSavedReportId ? "Edit Statement" : "New Statement";

  ownerNameInput.value = String(selected.ownerName || "");
  ownerEmailInput.value = String(selected.ownerEmail || "");
  propertyInput.value = String(selected.propertyName || "");
  dateInput.value = String(selected.statementDate || "");
  periodInput.value = String(selected.periodLabel || "");
  ownerTypeSelect.value = String(selected.ownerType || "payout");

  tbody.innerHTML = "";
  const rows = Array.isArray(selected.recurringCharges) ? selected.recurringCharges : [];
  if (!rows.length) {
    createRecurringExpenseRow({ name: "", amount: 0 });
  } else {
    rows.forEach(item => createRecurringExpenseRow(item));
  }
  updateRecurringTotalDisplay();

  ownerStayTbody.innerHTML = "";
  const ownerStayRows = Array.isArray(selected.ownerStayFees) ? selected.ownerStayFees : [];
  if (ownerStayRows.length) {
    ownerStayRows.forEach(item => createOwnerStayFeeRow(item));
  } else if (String(selected.ownerType || "payout") === "payout") {
    const scanned = getScannedOwnerStayFeesFromReservations();
    if (scanned.length) {
      scanned.forEach(item => createOwnerStayFeeRow(item));
    } else {
  ...
  updateOwnerStayTotalDisplay();
  toggleOwnerStaySection(selected.ownerType, !!report);
}

async function saveStatementReport() {
  const report = getStatementEditorValues();
  if (!report) return;

  if (!report.ownerName || !report.statementDate) {
    alert("Statement date and owner name are required.");
    return;
  }

  const existing = savedReportsData.find(item => item.id === report.id);
  const payload = {
    ...report,
  ...
  const apiBase = resolvedSavedReportsApiBase || SAVED_REPORTS_API_CANDIDATES[0];
  if (savedReportsPersistenceMode === "server" && apiBase) {
    try {
      const method = existing ? "PUT" : "POST";
      const target = existing ? `${apiBase}/${encodeURIComponent(payload.id)}` : apiBase;
      ...
      savedReportsData = await fetchSavedReportsFromApi();
      saveSavedReportsToStorage();
      activeSavedReportId = payload.id;
      renderSavedReportsList();
      populateStatementEditor(savedReportsData.find(item => item.id === payload.id) || payload);
      alert("Report saved.");
      return;
    } catch (_) {
      savedReportsPersistenceMode = "local";
  ...
  const idx = savedReportsData.findIndex(item => item.id === payload.id);
  if (idx === -1) {
    savedReportsData.unshift(payload);
  } else {
    savedReportsData[idx] = payload;
  }
  saveSavedReportsToStorage();
  activeSavedReportId = payload.id;
  renderSavedReportsList();
  populateStatementEditor(payload);
  alert("Report saved.");
}

async function deleteStatementReport(reportId) {
  if (!reportId) return;
  const confirmDelete = window.confirm("Delete this saved report?");
  if (!confirmDelete) return;

  const apiBase = resolvedSavedReportsApiBase || SAVED_REPORTS_API_CANDIDATES[0];
  if (savedReportsPersistenceMode === "server" && apiBase) {
    try {
      const res = await fetch(`${apiBase}/${encodeURIComponent(reportId)}`, {
        method: "DELETE",
        headers: { accept: "application/json" }
      });
      ...
      savedReportsData = await fetchSavedReportsFromApi();
      saveSavedReportsToStorage();
      if (activeSavedReportId === reportId) populateStatementEditor(null);
      renderSavedReportsList();
      return;
    } catch (_) {
      savedReportsPersistenceMode = "local";
  ...
  savedReportsData = savedReportsData.filter(item => item.id !== reportId);
  saveSavedReportsToStorage();
  if (activeSavedReportId === reportId) populateStatementEditor(null);
  renderSavedReportsList();
}

function buildSavedReportLink(reportId) {
  const url = new URL(window.location.href);
  url.searchParams.set("savedReportId", reportId);
  return url.toString();
}

function openSavedReportForEditing(reportId) {
  const report = savedReportsData.find(item => item.id === reportId);
  if (!report) return;
  populateStatementEditor(report);
}

function attemptLoadSavedReportFromQuery() {
  if (hasAttemptedSavedReportQueryLoad) return;
  hasAttemptedSavedReportQueryLoad = true;
  const params = new URLSearchParams(window.location.search || "");
  const reportId = String(params.get("savedReportId") || "").trim();
  if (!reportId) return;
  const report = savedReportsData.find(item => item.id === reportId);
  if (report) {
    openSavedReportForEditing(reportId);
  }
}
...
function renderSavedReportsList() {
  ...
  const sorted = [...savedReportsData].sort((a, b) => {
    return new Date(b.updatedAt || b.createdAt).getTime() - new Date(a.updatedAt || a.createdAt).getTime();
  });

  if (!sorted.length) {
    tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;">No saved reports yet.</td></tr>`;
    return;
  }

  tbody.innerHTML = sorted.map(report => {
    const createdLabel = new Date(report.createdAt || Date.now()).toLocaleString();
    return `
      <tr>
        <td>${escapeHtml(createdLabel)}</td>
        <td>${escapeHtml(report.ownerName || "")}</td>
        <td>${escapeHtml(report.reportName || "Statement")}</td>
        <td>${escapeHtml(report.periodLabel || "")}</td>
        <td>${escapeHtml((report.ownerType || "payout").toUpperCase())}</td>
        <td class="statement-action-cell">
          <button type="button" class="statement-mini-btn statement-mini-btn-open" data-action="open" data-id="${escapeHtml(report.id)}">Open</button>
          <button type="button" class="statement-mini-btn statement-mini-btn-copy" data-action="copy" data-id="${escapeHtml(report.id)}">Copy Link</button>
          <button type="button" class="statement-mini-btn statement-mini-btn-danger" data-action="delete" data-id="${escapeHtml(report.id)}">Delete</button>
          <button type="button" class="statement-mini-btn statement-mini-btn-email" data-action="email" data-id="${escapeHtml(report.id)}">Email</button>
        </td>
      </tr>
    `;
  }).join("");
  ...
  tbody.querySelectorAll("button[data-action]").forEach(btn => {
    ...
    btn.onclick = async () => {
      if (action === "open") {
        openSavedReportForEditing(id);
        return;
      }

      if (action === "copy") {
        const link = buildSavedReportLink(id);
        try {
      ...
      if (action === "delete") {
        await deleteStatementReport(id);
        return;
      }

      if (action === "email") {
        const report = savedReportsData.find(item => item.id === id);
        const toEmail = encodeURIComponent(String(report?.ownerEmail || ""));
        const subject = encodeURIComponent(`Owner Statement ${report?.periodLabel || ""}`.trim());
        const body = encodeURIComponent(`Open your saved statement here:\n${buildSavedReportLink(id)}`);
...
function renderStatementWorkspace(container) {
  if (!container) return;
  let wrap = document.getElementById("statementWorkspaceWrap");
  if (!wrap) {
    wrap = document.createElement("div");
    wrap.id = "statementWorkspaceWrap";
    wrap.className = "statement-workspace-wrap";
    container.appendChild(wrap);
  }

  wrap.innerHTML = `
    <div class="statement-editor-card">
      <h2 id="statementEditorTitle" class="section-title" style="text-align:center; margin:0 0 14px 0;">New Statement</h2>
      <div class="statement-top-grid">
        <label class="statement-label">Statement Date<input id="statementDateInput" type="date" class="statement-input" /></label>
        <label class="statement-label">Owner Name<input id="statementOwnerNameInput" type="text" class="statement-input" /></label>
        <label class="statement-label">Owner Email<input id="statementOwnerEmailInput" type="email" class="statement-input" /></label>
        <label class="statement-label">Property<input id="statementPropertyNameInput" type="text" class="statement-input" /></label>
        <label class="statement-label">Period<input id="statementPeriodInput" type="text" class="statement-input" placeholder="March 2026" /></label>
        <label class="statement-label">Owner Type
          <select id="statementOwnerTypeSelect" class="statement-input">
      ...
      <h3 class="section-title" style="margin:18px 0 8px 0;">Recurring Expenses</h3>
      <div class="table-wrap">
        <table>
          <thead>
            <tr>
              <th style="text-align:left;">NAME OF EXPENSE</th>
              <th style="text-align:left;">AMOUNT</th>
              <th style="text-align:left;">ACTIONS</th>
            </tr>
          </thead>
          <tbody id="recurringExpensesBody"></tbody>
          <tfoot>
            <tr>
              <th style="text-align:left;">TOTAL</th>
              <th style="text-align:left;" id="recurringExpensesTotal">$0.00</th>
              <th></th>
            </tr>
          </tfoot>
        </table>
      ...
      <div id="ownerStayFeesSection">
        <h3 class="section-title" style="margin:18px 0 8px 0;">Owner Stays (Cleaning Fee Only)</h3>
        <div class="table-wrap">
          <table>
            <thead>
              <tr>
                <th style="text-align:left;">OWNER STAY</th>
                <th style="text-align:left;">CLEANING FEE</th>
                <th style="text-align:left;">ACTIONS</th>
              </tr>
            </thead>
            <tbody id="ownerStayFeesBody"></tbody>
            <tfoot>
              <tr>
                <th style="text-align:left;">TOTAL</th>
                <th style="text-align:left;" id="ownerStayFeesTotal">$0.00</th>
                <th></th>
              </tr>
            </tfoot>
          </table>
      ...
      <div class="statement-toolbar">
        <button id="statementAddRecurringBtn" type="button" class="statement-main-btn statement-main-btn-secondary">Add Expense</button>
        <button id="statementAddOwnerStayBtn" type="button" class="statement-main-btn statement-main-btn-secondary">Add Owner Stay</button>
        <button id="statementScanOwnerStaysBtn" type="button" class="statement-main-btn statement-main-btn-secondary">Scan Owner Stays</button>
        <button id="statementNewBtn" type="button" class="statement-main-btn statement-main-btn-ghost">New</button>
        <button id="statementSaveBtn" type="button" class="statement-main-btn statement-main-btn-primary">Save</button>
      </div>
    </div>

    <div class="statement-saved-card">
      <h2 class="section-title" style="text-align:center; margin:0 0 14px 0;">Saved Reports</h2>
      <div class="table-wrap">
        <table>
          <thead>
            <tr>
  ...
  document.getElementById("statementAddRecurringBtn").onclick = () => createRecurringExpenseRow({ name: "", amount: 0 });
  document.getElementById("statementAddOwnerStayBtn").onclick = () => {
    createOwnerStayFeeRow({ label: "", cleaningFee: Number(getCleaningFee()) || 0 });
    updateOwnerStayTotalDisplay();
  };
  document.getElementById("statementScanOwnerStaysBtn").onclick = () => refillOwnerStayFeesFromScan();
  document.getElementById("statementNewBtn").onclick = () => populateStatementEditor(null);
  document.getElementById("statementSaveBtn").onclick = () => saveStatementReport();
  document.getElementById("statementOwnerTypeSelect").onchange = () => {
    const ownerType = String(document.getElementById("statementOwnerTypeSelect")?.value || "payout");
    toggleOwnerStaySection(ownerType, !!activeSavedReportId);
  };

  populateStatementEditor(null);
  renderSavedReportsList();
  ensureSavedReportsInitialized();
}
...
if (currentOwner && currentOwner.admin) {
  ...
  if (isAdminReport) {
    ...
    renderStatementWorkspace(container);
    return;
  }
